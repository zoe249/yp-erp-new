  ...
  <div ref="appRef" class="container-box">
    ...
    >
      ...
      <template #storeCode="{ row }">
        <span>{{ spliceCodeName(row.storeCode, row.storeCodeDescr) }}</span>
      </template>
      <template #type="{ row }">
        <span>{{ spliceCodeName(row.type, row.typeDescr) }}</span>
      </template>
      <template #storageAreaCode="{ row }">
        <span>{{ spliceCodeName(row.storageAreaCode, row.storageAreaCodeDescr) }}</span>
      </template>
      <template #status="{ row }">
...
<script lang="ts" setup name="StoreStorage">
